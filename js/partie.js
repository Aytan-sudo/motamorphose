...
export function creerPartie(paire) {
  return { ...paire, chemin: [paire.depart], indices: 0, retours: 0, terminee: false };
}
...
export function annuler(partie) {
  ...
  return { ...partie, chemin: partie.chemin.slice(0, -1), retours: partie.retours + 1 };
}

export function demanderIndice(partie, graphe) {
  if (partie.terminee) return { partie, mot: null };
  if (partie.indices >= MAX_INDICES) return { partie, mot: null, limite: true };
...
export function score(partie) {
  return Math.max(0, 100 - (partie.chemin.length - 1 - partie.optimal) * 5 - partie.indices * 15 - partie.retours * 3);
}
